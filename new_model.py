...
for i,(k,v) in enumerate(example_data.items()):
    ...
    #wenn mind zwei dasselbe gelabled haben, dann erst hinzufügen
    #entferne token, wenn nicht uebereinstimmt
    #k fold implementieren
...
embedding_layer = Embedding(vocab_size, 300, weights=[embedding_vectors], input_length=max_seq_length, trainable= True)(input_layer)
#trainable = true -> trainiert gewichtungsmatrix von embeddings neu (statt wiki)

# here, attention models have to be implemented in this model
# ...
# -> lstm etc. (hidden layers)
